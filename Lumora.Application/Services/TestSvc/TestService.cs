...
{
    public class TestService(ITestRepository repository, ILogger<TestService> logger, TestMessage messages, IUnitOfWork unitOfWork) : ITestService
    {
        ...
        public async Task<GeneralResult<int>> CreateTestAsync(TestCreateDto dto, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var existingActiveTest = await _repository.GetActiveTestForLessonAsync(dto.LessonId, cancellationToken);
                ...
                var lesson = await _repository.GetLessonAsync(dto.LessonId, cancellationToken);
                if (lesson == null)
                {
                ...
                foreach (var qDto in dto.Questions)
                {
                    ...
                    await _repository.AddTestQuestionAsync(question, cancellationToken);
                    ...
                    foreach (var cDto in qDto.Choices)
                    {
                        ...
                        await _repository.AddTestChoiceAsync(choice, cancellationToken);
                    }
                }
        ...
        public async Task<GeneralResult> UpdateTestAsync(int testId, TestUpdateDto dto, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var test = await _repository.GetTestByIdAsync(testId, true, cancellationToken);
        ...
        public async Task<GeneralResult<PagedResult<TestDetailsDto>>> GetAllTestsAsync(PaginationRequestDto pagination, CancellationToken cancellationToken)
        {
            try
            {
                var pagedEntities = await _repository.GetPagedTestsAsync(pagination, cancellationToken);
                ...
                _logger.LogInformation("TestService - GetAllTestsAsync : Retrieved {Count} tests.", dtos.Count);
