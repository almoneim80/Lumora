namespace Lumora.Application.Helpers
{
    public class FileValidatorHelper
    {
        ...
        public (bool Success, string? ErrorMessage, FileValidationErrorType ErrorType) Validate(IFileStream file, MediaType type)
        {
            if (file == null)
            ...
            if (category.MaxSizePerExtension == null)
            {
                return (false, $"Invalid max size format in config.", FileValidationErrorType.InvalidSizeFormat);
            }
