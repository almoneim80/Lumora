namespace Lumora.Infrastructure.Persistence.Queries
{
    public class ESOnlyQueryProviderFactory<T> : QueryProviderFactory<T>
        where T : BaseEntityWithId, new()
    {
        public ESOnlyQueryProviderFactory(
            PgDbContext dbContext, EsDbContext esDbContext, IOptions<ApiSettingsConfig> apiSettingsConfig,
            IHttpContextHelper? httpContextHelper, IConfiguration configuration)
        ...
        public override IQueryProvider<T> BuildQueryProvider(int limit = -1)
        {
            ...
            var indexPrefix = dbContext.Configuration.GetSection("Elastic:IndexPrefix").Value;
