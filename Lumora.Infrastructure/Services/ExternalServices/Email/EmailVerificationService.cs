namespace Lumora.Infrastructure.Services.ExternalServices.Email
{
    ...
    public class EmailVerificationService : ControllerBase, IEmailVerificationService
    {
        ...
        public async Task<(bool Succeeded, string Message, IEnumerable<string>? ErrorMessages)> ConfirmEmailAsync(string userId, string token)
        {
            try
            {
                ...
                else
                {
                    ...
                    return (false, "Email confirmation failed.", result.Errors.Select(e => e.Description));
                }
            }
            catch (Exception ex)
            {
