namespace Lumora.Infrastructure.Services.ExternalServices.Email;
...
public class MxVerifyService : IMxVerifyService
{
    ...
    public async Task<MxCheckResultDto[]> BulkVerify(string mxValue, params string[] emails)
    {
        await SemaphoreSlim.WaitAsync();

        var port = 25;
        var returnResults = new List<MxCheckResultDto>();

        MxCheckResultDto[] ReturnAllFailedWithCode(int code, string value)
        {
            return emails
                .Select(e => new MxCheckResultDto(mxValue, e, code, value, false))
                .ToArray();
        }
        ...
        try
        {
            ...
            foreach (var email in emails)
            {
                ...
                switch (rc.c)
                {
                    // 250 - ok
                    case 250:
                        returnResults.Add(new MxCheckResultDto(mxValue, email, MxResultCode.Ok, string.Empty, true));
                        break;

                    // 450 - user got to many mails, but exist
                    case 450:
                        returnResults.Add(new MxCheckResultDto(mxValue, email, MxResultCode.OkButTooManyEmailsGot, string.Empty, true));
                        break;

                    // 452 - user mailbox full, but exist
                    case 452:
                        returnResults.Add(new MxCheckResultDto(mxValue, email, MxResultCode.OkButMailboxFull, string.Empty, true));
                        break;

                    // Other response codes should be treated as a failure code.
                    default:
                        returnResults.Add(new MxCheckResultDto(mxValue, email, rc.c, rc.v, false));
                        break;
                }
            }
        ...
        catch (Exception ex)
        {
            ...
            var notProcessed = emails
                ...
                .Select(e => new MxCheckResultDto(mxValue, e, MxResultCode.GeneralException, ex.Message, false))
                .ToArray();
