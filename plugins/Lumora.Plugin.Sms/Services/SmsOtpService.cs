        ...
        ICacheService cacheService) : IOtpService
    {
        ...
        public async Task<GeneralResult> GenerateAndSendOtpAsync(string phoneNumber)
        {
            try
            {
                ...
                return new GeneralResult(true, "OTP sent successfully.", null, Domain.Enums.ErrorType.Success);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to generate or send OTP for {PhoneNumber}.", phoneNumber);
                return new GeneralResult(false, "Failed to generate or send OTP.", null, Domain.Enums.ErrorType.InternalServerError);
            }
        }
        ...
        public async Task<GeneralResult> ResendOtpAsync(string phoneNumber)
        {
            try
            {
                ...
                return new GeneralResult(true, "OTP re-sent successfully.", null, Domain.Enums.ErrorType.Success);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to resend OTP for {PhoneNumber}.", phoneNumber);
                return new GeneralResult(false, "Feiled to resend OTP.", null, Domain.Enums.ErrorType.InternalServerError);
            }
        }
        ...
        public async Task<GeneralResult<bool>> VerifyOtpAsync(string phoneNumber, string inputOtp)
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(inputOtp))
                {
                    ...
                    return new GeneralResult<bool>(false, "Invalid OTP.", false, Domain.Enums.ErrorType.BadRequest);
                }
                ...
                if (storedOtp == null)
                {
                    ...
                    return new GeneralResult<bool>(false, "OTP expired or not found.", false, Domain.Enums.ErrorType.BadRequest);
                }
                ...
                return new GeneralResult<bool>(true, "OTP verified successfully.", true, Domain.Enums.ErrorType.Success);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to verify OTP for {PhoneNumber}.", phoneNumber);
                await InvalidateOtpAsync(phoneNumber);
                return new GeneralResult<bool>(false, "Feailed to verify OTP.", false, Domain.Enums.ErrorType.InternalServerError);
            }
        }
