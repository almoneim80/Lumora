    ...
    public class SmsService : ISmsService
    {
        ...
        public async Task<GeneralResult> SendAsync(string recipient, string message)
        {
            try
            {
                ...
                if (smsService == null)
                {
                    return new GeneralResult(false, "SMS service not found.", null, Domain.Enums.ErrorType.BadRequest);
                }
            ...
            catch (Exception ex)
            {
                ...
                return new GeneralResult(false, "Failed to send SMS.", null, Domain.Enums.ErrorType.InternalServerError);
            }
        }
        ...
        public GeneralResult GetSender(string recipient)
        {
            try
            {
                ...
                return new GeneralResult(true, "Sender found.", smsService.GetSender(recipient), Domain.Enums.ErrorType.Success);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to get sender.");
                return new GeneralResult(false, "Failed to get sender.", null, Domain.Enums.ErrorType.InternalServerError);
            }
        }
        ...
        public async Task<GeneralResult> AddSmsToDb(CreateSmsDto dto, CancellationToken cancellationToken)
        {
            try
            {
                if (dto == null)
                {
                    ...
                    return new GeneralResult(false, "all fields are required", null, Domain.Enums.ErrorType.BadRequest);
                }
                ...
                if (result == 0)
                {
                    ...
                    return new GeneralResult(false, "Failed to add SMS to database.", null, Domain.Enums.ErrorType.InternalServerError);
                }
                else
                {
                    _logger.LogInformation("SMS added to database successfully.");
                    return new GeneralResult(true, "SMS added to database successfully.", null, Domain.Enums.ErrorType.Success);
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<GeneralResult<List<SmsLog>>> GetAllSms(CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (!smsLogs.Any())
                {
                    ...
                    return new GeneralResult<List<SmsLog>>(false, "No SMS found.", null, Domain.Enums.ErrorType.NotFound);
                }

                _logger.LogInformation("All SMS found.");
                return new GeneralResult<List<SmsLog>>(true, "All SMS found.", smsLogs, Domain.Enums.ErrorType.Success);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to get all SMS.");
                return new GeneralResult<List<SmsLog>>(false, "Failed to get all SMS.", null, Domain.Enums.ErrorType.InternalServerError);
            }
        }
        ...
        public async Task<GeneralResult<SmsLog>> GetSmsById(int id, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (smsLog == null)
                {
                    ...
                    return new GeneralResult<SmsLog>(false, "SMS not found.", null, Domain.Enums.ErrorType.NotFound);
                }

                _logger.LogInformation("SMS found.");
                return new GeneralResult<SmsLog>(true, "SMS found.", smsLog, Domain.Enums.ErrorType.Success);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to get SMS.");
                return new GeneralResult<SmsLog>(false, "Failed to get SMS.", null, Domain.Enums.ErrorType.InternalServerError);
            }
        }
        ...
        public async Task<GeneralResult> SuccessSent(int? smsLogId, CancellationToken cancellationToken)
        {
            try
            {
                if (smsLogId == null)
                {
                    ...
                    return new GeneralResult(false, "SMS log ID is required.", null, Domain.Enums.ErrorType.BadRequest);
                }
                ...
                if (smsLog == null)
                {
                    ...
                    return new GeneralResult(false, "SMS log not found.", null, Domain.Enums.ErrorType.NotFound);
                }
                ...
                if (result == 0)
                {
                    ...
                    return new GeneralResult(false, "Failed to update SMS status.", null, Domain.Enums.ErrorType.InternalServerError);
                }

                _logger.LogInformation("SMS status updated successfully.");
                return new GeneralResult(true, "SMS status updated successfully.", null, Domain.Enums.ErrorType.Success);
            }
            catch (Exception ex)
            {
