        ...
        ICascadeDeleteService cascadeDeleteService) : AuthenticatedController
    {
        ...
        public async Task<IActionResult> ChangePhoneNumber([FromBody] ChangePhoneNumberDto dto, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var result = await _userProfileService.ChangePhoneNumberAsync(CurrentUserId!, dto.PhoneNumber, cancellationToken);
        ...
        public async Task<IActionResult> UpdateProfile([FromForm] UserUpdateFormDto dto, CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    var fileResult = dto.AvatarFile.PrepareValidatedFile(Domain.Enums.MediaType.Image, _fileValidator);
                ...
                var result = await _userProfileService.UpdateProfileAsync(CurrentUserId!, updateDto, cancellationToken);
                if (result.IsSuccess == false)
                {
                    return result.ErrorType switch
                    {
        ...
        public async Task<IActionResult> CompleteProfile([FromForm] CompleteUserDataFormDto dto, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (dto.AvatarFile is not null)
                {
                    // الآن الـ Extension method ستعمل لأن dto.AvatarFile لم يعد null
                    var fileResult = dto.AvatarFile.PrepareValidatedFile(Domain.Enums.MediaType.Image, _fileValidator);
                ...
                var result = await _userProfileService.CompleteProfileAsync(CurrentUserId!, completeDto, cancellationToken);
                if (result.IsSuccess == false)
                {
                    return result.ErrorType switch
                    {
        ...
        [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> MyData(CancellationToken cancellationToken, PaginationRequestDto pagination)
        {
            try
            {
                var userCheck = CheckUserOrUnauthorized();
                ...
                var result = await _userProfileService.GetProfileAsync(CurrentUserId!, cancellationToken, pagination);
                if (result.IsSuccess == false)
                {
                    return result.ErrorType switch
                    {
