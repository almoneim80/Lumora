using Lumora.Infrastructure.Files;
            ...
            ILogger<ProgramCourseController> logger) : AuthenticatedController
    {
        ...
        public async Task<IActionResult> Create([FromForm] CourseWithLessonsCreateFormDto formDto, CancellationToken cancellationToken)
        {
            try
            {
                ...
                try
                {
                    ...
                    {
                        var fileResult = formDto.CourseLogo.PrepareValidatedFile(Domain.Enums.MediaType.Image, _fileValidator);
                    ...
                    if (dto.Lessons is not null)
                    {
                        for (int i = 0; i < dto.Lessons.Count; i++)
                        {
                            ...
                            {
                                IFileStream wrappedFile = new FormFileWrapper(formFile);
                                var fileResult = wrappedFile.PrepareValidatedFile(Domain.Enums.MediaType.Video, _fileValidator);
                                if (!fileResult.IsValid)
                                {
                            ...
                            if (dto.Lessons[i].Attachments is not null)
                            {
                                for (int j = 0; j < dto.Lessons[i].Attachments.Count; j++)
                                {
                                    ...
                                    {
                                        IFileStream wrappedFile = new FormFileWrapper(attachFile);
                                        var attachResult = wrappedFile.PrepareValidatedFile(Domain.Enums.MediaType.Document, _fileValidator);
        ...
        public async Task<IActionResult> Update([FromQuery] int courseId, [FromForm] CourseUpdateFormDto formDto, CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    var fileResult = formDto.Logo.PrepareValidatedFile(Domain.Enums.MediaType.Image, _fileValidator);
                    if (!fileResult.IsValid)
                    {
