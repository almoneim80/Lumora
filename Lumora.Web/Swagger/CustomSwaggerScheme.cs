...
namespace Lumora.Web.Swagger
{
    public class CustomSwaggerScheme : ISchemaFilter
    {
        ...
        {
            if (context.Type.Namespace!.Contains("Lumora") && schema.Properties.Any())
            {
        ...
        private static void SetDictionaryArrayExample(PropertyInfo property, OpenApiSchema schema)
        {
            ...
            var data = attr != null
                ? attr.Value.ToDictionary(x => x.Key, x => (IOpenApiAny)new OpenApiString(x.Value))
                : new Dictionary<string, IOpenApiAny>() { { "key1", new OpenApiString("value1") }, { "key2", new OpenApiString("value2") } };

            foreach (var item in data)
            {
                if (!openApiObject.ContainsKey(item.Key))
                {
                    openApiObject.Add(item.Key, item.Value);
                }
            }
